...
const tfcCalendar = Java.loadClass('net.dries007.tfc.util.calendar.Calendars') //requires kubejs/config/common.json with disableClassFilter = true
...
BlockEvents.rightClicked(event => 
{
    ...
    if (event.item == 'minecraft:flint_and_steel' || event.item == 'tfc:firestarter'){
        ...
        if (event.block.properties.heat_level == '0'){
            ...
            if (!(event.block.north.hasTag('tfc:charcoal_forge_insulation') && 
                event.block.south.hasTag('tfc:charcoal_forge_insulation') && 
                event.block.east.hasTag('tfc:charcoal_forge_insulation') && 
                event.block.west.hasTag('tfc:charcoal_forge_insulation') && 
                event.block.down.hasTag('tfc:charcoal_forge_insulation'))){
                return
            }
